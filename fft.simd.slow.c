...
// (a + i*b) * (c + i*d) = (e + i*f)
inline void cpx_mul(double a, double b, double c, double d, double* e, double* f) {
    *e = a * c - b * d;
    *f = a * d + b * c;
}

// (a + i*b) * (c + i*d) = (e + i*f)
inline void cpx_mul_128(__m128d a, __m128d b, __m128d c, __m128d d, __m128d* e, __m128d* f) {
...
// (a + i*b) * (c + i*d) = (e + i*f)
inline void cpx_mul_256(__m256d a, __m256d b, __m256d c, __m256d d, __m256d* e, __m256d* f) {
...
// (a + i*b) * (c + i*d) = (e + i*f)
inline void cpx_mul_512(__m512d a, __m512d b, __m512d c, __m512d d, __m512d* e, __m512d* f) {
...
void fft(double* a_r, double* a_i, u64 n, u64 invert) {
    ...
    if (invert) {
        assert(n % 8 == 0);
        __m512d n_rr = _mm512_set1_pd((double)1 / n);
        __m512d n_ii = _mm512_set1_pd(0);
        for (u64 i = 0; i < n; i += 8) {
            __m512d a_rr = _mm512_loadu_pd(&a_r[i]);
            __m512d a_ii = _mm512_loadu_pd(&a_i[i]);
            cpx_mul_512(a_rr, a_ii, n_rr, n_ii, &a_rr, &a_ii);
            _mm512_storeu_pd(&a_r[i], a_rr);
            _mm512_storeu_pd(&a_i[i], a_ii);
        }
    }
}
...
BigInt mul(BigInt a, BigInt b) {
    ...
    assert(n % 8 == 0);
    for (u64 i = 0; i < n; i += 8) {
        __m512d a_r = _mm512_loadu_pd(&fa_r[i]);
        __m512d a_i = _mm512_loadu_pd(&fa_i[i]);
        __m512d b_r = _mm512_loadu_pd(&fb_r[i]);
        __m512d b_i = _mm512_loadu_pd(&fb_i[i]);
        cpx_mul_512(a_r, a_i, b_r, b_i, &a_r, &a_i);
        _mm512_storeu_pd(&fa_r[i], a_r);
        _mm512_storeu_pd(&fa_i[i], a_i);
    }
    fft(fa_r, fa_i, n, 1);
